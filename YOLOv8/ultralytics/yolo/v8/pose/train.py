...
def train(cfg=DEFAULT_CFG, use_python=False):
    ...
    model = cfg.model or 'yolov8s-pose.pt'
    data = cfg.data or 'test_wrinkle.yaml'  # or yolo.ClassificationDataset("mnist")
    device = cfg.device if cfg.device is not None else ''
    batch = 8;
    workers = 0;
    imgsz = 320;
    ...
    args = dict(model=model, data=data, device=device, batch=batch, workers=workers,imgsz = imgsz,epochs = epochs, patience=0)
    if use_python:
